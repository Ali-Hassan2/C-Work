...
class login{
	...
	if(write.is_open())
	{
		...
		write.close();
		cout<<"Your account created successfully please wait";
	for(int i=0;i<6;i++)
	...
	void inputasuser(void)
	{
		...
		if(write.is_open())
		{
			write << "User username : "<<username <<endl;
			write << "User password : "<<upassword<<endl;
			write.close();
			//cout<<"Data has successfully stored.."<<endl;
		}		
		else
		{
	...
	bool loginuser(void)
	{
		...
		cout<<"\n\n\nEnter the User username : "<<endl;
		cin>>uu;
		cout<<"\n\n\nEnter the User password : "<<endl;
		cin>>up;
		 ifstream read;
		 read.open("E:\\login1.txt");
		 if(read.is_open())
