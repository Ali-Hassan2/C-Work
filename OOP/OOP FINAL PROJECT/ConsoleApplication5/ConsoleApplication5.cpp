...
/// Event Registration system in c++
...
public:
    ...
    void writetofile(Event_Management& event) {
        if (count < MAX_EVENTS) {
            event.eid = ++idcount;
            events[count] = event;
            count++;
...
void Event_Management::addEvent() { 
    ...
    eventCollection.writetofile(*this);
        ...
        if (count < max_org) {
            ...
            if (write.is_open()) {
                for (int i = 0; i < count; i++) {
                    ...
                    int eventIndex = eventCollection.getEventByID(org.associated_event_id);
                    if (eventIndex != -1) {
                        org.ptrevent = &eventCollection.events[eventIndex];
                        write << "Associated Event: " << org.ptrevent->name << endl;
                    }
                    else {
                        write << "Not Associated with an Event" << endl;
                    }

                    write << "------------------------------------" << endl;
                }
                write.close();
                cout << "Assigned Organizer ID: " << org.oid << endl;
            ...
            cout << "There is no space to accommodate..." << endl;
        }
...
void Event_Organizer::addOrganizer() {
    ...
    HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
    SetConsoleTextAttribute(hConsole, FOREGROUND_GREEN);
    organizercollection oc;

    cout << "\t\t-------* Enter Organizer Details *---------\n" << endl;
    cout << endl << endl;

    cout << "Enter the Name of Organizer: ";
    name = Utility::GetStringFromUser(12);
    cout << "Enter the Phone Number: ";
    phone = Utility::GetNumberFromUser(12);
    cout << "Enter the Email: ";
    email = Utility::GetStringFromUser(20);
    cout << "Enter the Registration number: ";
    registration_number = Utility::GetNumberFromUser(10);
    cout << "Enter the date of birth (dd mm yyyy): ";
    cin >> date_of_birth.dd >> date_of_birth.mm >> date_of_birth.yyyy;

    // New code: Ask for the ID of the associated event
    cout << "Enter the ID of the associated event: ";
    associated_event_id = Utility::GetNumberFromUser(3);

    // Add the organizer to the collection and associate with the event
    oc.addorganizer(*this);
    ...
    if (!read.is_open()) {
        ...
        while (getline(read, line)) {
            if (line.find("ID: ") != string::npos) {
                ...
                else if (ptrevent != nullptr) {
                    cout << "Associated Event: " << ptrevent->name << endl;
                }
                else {
    ...
    if (file.is_open()) {
        ...
        while (getline(file, line)) {
            ...
            if (pos != string::npos) {
                ...
                cout << "Enter the ID of the associated event: ";
                associated_event_id = Utility::GetNumberFromUser(1);
        ...
        if (count < max_size)
        {
            ...
            else
            {
                for (int i = 0; i < count; i++)
                {
                    ...
                    int eventIndex = eventCollection.getEventByID(att.associated_event_id);
                    if (eventIndex != -1) {
                        att.ptrevent = &eventCollection.events[eventIndex];
                        write << "Associated Event: " << att.ptrevent->name << endl;
                    }
                    else {
                        write << "Not Associated with an Event" << endl;
                    }
                }
                cout << "Assigned Attendee ID: " << att.aid << endl;
...
void Attendee::addAttendee()
{
    ...
    cout << "------------------\n" << endl;
    cout << "Enter the Name : ";
    name = Utility::GetStringFromUser(15);
    cout << "Enter the Phone : ";
    phone = Utility::GetNumberFromUser(15);
    cout << "Enter the email : ";
    email = Utility::GetStringFromUser(20);
    cout << "Special request(y/n) :";
    special_request = Utility::GetStringFromUser(1);
    cout << "Enter the ID of the associated event: ";
    associated_event_id = Utility::GetNumberFromUser(3);
        ...
        if (count < max_size)
        {
            ...
            else
            {
                for (int i = 0; i < count; i++)
                {
                    ...
                    int eventIndex = attendeeCollection.getEventByID(tic.associated_attendee_id);
                    if (eventIndex != -1) {
                        tic.ptrattendee = &attendeeCollection.attendee[eventIndex];
                        write << "Associated Attendee: " << tic.ptrattendee->name << endl;
                    }
                    else {
                        write << "Not Associated with an Attendee" << endl;
                    }
                }
                cout << "Assigned Ticket ID: " << tic.tid << endl;
                cout << "Data Saved Successfully..." << endl;
                write.close();
            }
        }
...
void Ticket::addTicket() {
    ...
    cin >> d.dd >> d.mm >> d.yyyy;
    cout << "Enter the ID of the associated Attendeer: ";
    associated_attendee_id = Utility::GetNumberFromUser(3);
    tc.addticket(*this);
    SetConsoleTextAttribute(hConsole, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
    ...
    if (!read.is_open()) {
        ...
        while (getline(read, line)) {
            if (line.find("ID: ") != string::npos) {
                ...
                else if (ptrattendee != nullptr) {
                    cout << "Associated Attendee: " << ptrattendee->name << endl;
                }
                else {
                    cout << "Not Associated with an Attednee.." << endl;
                }
        ...
        if (count < max_size)
        {
            ...
            else
            {
                for (int i = 0; i < count; i++)
                {
                    ...
                    int eventIndex = eventCollection.getEventByID(sch.associated_event_id);
                    if (eventIndex != -1) {
                        sch.ptrevent = &eventCollection.events[eventIndex];
                        write << "Associated Event: " << sch.ptrevent->name << endl;
                    }
                    else {
                        write << "Not Associated with an Event" << endl;
                    }
                ...
                cout << "Assigned Schedule ID: " << sch.sid << endl;
                cout << "Data Saved Successfully.." << endl;
                write.close();
            }
        }
        ...
        case 2:
        {
            ...
            if (l.loginadmin())
            {
                ...
                do
                {
                    ...
                    cout << " \t\t\t\t* 2. ORGANIZER MANAGEMENT               *" << endl;
                    cout << " \t\t\t\t* 3. ATTENDEE MANAGEMENT                *" << endl;
                    cout << " \t\t\t\t* 4. TICKET MANAGEMENT                  *" << endl;
                    cout << " \t\t\t\t* 5. SCHEDULE MANAGEMENT                *" << endl;
                    cout << " \t\t\t\t* 0. EXIT                               *" << endl;
                    cout << "\t\t\t\t******************************************" << endl;
